...
(function() {
    ...
    var app = angular.module('dragndrop.controller', []);

    app.controller('DragndropController', ['$scope', '$window', function($scope, $window){
        ...
        $scope.displaySrcImage = function(image) {
        ...
        $scope.localFinished = function(id, name, json) {
            // debugger;
            $scope.displaySimilarImages(json.sourceSig, json.images);
        ...
        $scope.imageURLToBlob = function(url) {
            // debugger;
        ...
        $scope.uploadOneFile = function(file, indice, nbFiles) {
            var xhr = new XMLHttpRequest();
            ...
            xhr.onreadystatechange = function (e) {
                if (xhr.readyState == 4) {
                    ...
                    if (xhr.status == 200) {
                        //      debugger;
//                    alert("finished!");
                        $scope.localFinished(0, 0, jQuery.parseJSON(xhr.responseText));
                    }
                }
            };


            $scope.displaySrcImage(file);
            xhr.send(file);
        };
        ...
        /**
        $scope.dropHandler = function(event){
            console.log("hello");
        }**/
        ...
        $scope.dropHandler = function(event) {
            ...
            if (files.length == 0) {
                ...
                if (src.substring(0, 4).indexOf("http") == 0) {
                    ...
                    $scope.imageURLToBlob(src); //.done(processData);

                } else {
                    var blob = $scope.dataURItoBlob(src);
                    tab_files.push(blob);
                    $scope.uploadBlobs(tab_files);
                }
            } else {
        ...
        $scope.uploadBlobs = function(tab_files) {
            var filesLen = tab_files.length;
            var filenames = "";
            // debugger;
            ...
            for (var i = 0; i < filesLen; i++) {
                ...
                li.textContent = tab_files[i].name + " (" + $scope.bytesToSize(tab_files[i].size) + ") ";
                ...
                $scope.uploadOneFile(tab_files[i], i, filesLen);
                //  debugger;
                if (i + 1 < filesLen) {
        ...
        $scope.displaySimilarImages = function(sourceSignature, object) {
            $('#duplicate_upload_result').children().remove();
